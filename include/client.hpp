...
using serial_pool = threadsafe::list_balancer_ext<procs::async_serial_ptr>;
...
public:
    ...
    /// Future variant of previous async_connect.
    /// \param master_ip_. String IPv4 of remote redis server.
    /// \param master_port_. Remote redis server port nubb
    /// \param slave_pool_. List of srv_endpoint which include ip, port and preference of slave server (1-10)
    /// \return std::future<asio::error_code>. After future_connect() use - future wait
    /// method (of returned future) blocks execution until all connections have been
    /// established or an error occurs.
...
private:
    ...
    procs::async_serial_ptr _master_serial;
    ...
    void async_send_pipe(const query<CmdType> & q_, std::true_type)
    {
        ...
        _master_pipeline->push(q_.get_callback(), q_.as_string_ref());
    }
    ...
    void async_send_pipe(const query<CmdType> & q_, std::false_type)
    {
        if (_slave_pipeline_pool.empty())
            ...
            _slave_pipeline_pool.balanced_rand()->push(q_.get_callback(), q_.as_string_ref());
    }
    ...
    void async_send_serial(const query<CmdType, BType> & q_, std::false_type)
    {
        if (_slave_serial_pool.empty())
            ...
            _slave_serial_pool.balanced_rand()->push(q_);
    }
